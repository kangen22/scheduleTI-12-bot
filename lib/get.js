...
export let getTimeUntilNextLesson = async (currentLesson, date, schedule) => {
  const nextLesson = getNextLesson(schedule, date);
  if (!nextLesson) {
    return null; // Если следующего урока нет, возвращаем null
  }
  const startTimeString = nextLesson['start_time'];
  const startTime = new Date(`${date.toDateString()} ${startTimeString}:00`);
  const timeDifferenceInMs = startTime - date;
  const timeUntilNextLesson = Math.round(timeDifferenceInMs / 60000).toString();
  return timeUntilNextLesson;
};
