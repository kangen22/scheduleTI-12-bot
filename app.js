...
app.get("/timeleft", async (req, res) => {
  ...
  try {
    ...
    const timeLeftInMinutes = currentLesson
      ? await getTimeLeft(currentLesson, date)
      : null;
    if (timeLeftInMinutes && parseInt(timeLeftInMinutes) > 0) {
      res.send(`${timeLeftInMinutes} хвилин до кінця пари "${lessonName}"`);
    } else {
      const timeUntilNextLesson = await getTimeUntilNextLesson(
        currentLesson,
        date,
        model
      );
      if (timeUntilNextLesson) {
        res.send(
          `Перерва. Наступна пара "${nextLessonName}" почнеться через ${timeUntilNextLesson} хвилин`
        );
      } else {
